...
def train(config, seed: int = 0) -> float:
    ...
    X_train = pd.read_csv("datasets/splits/X_final_train.csv")
    Y_train = pd.read_csv("datasets/splits/Y_final_train.csv")
    X_test = pd.read_csv("datasets/splits/X_final_test.csv")
    Y_test = pd.read_csv("datasets/splits/Y_final_test.csv")
    for j in range(3):
        model = CopyNumberPredictor(config)
        model.fit(X_train,Y_train,verbose=False)
        pred = model.predict(X_test)
